...
def get_total_mounted_pieces_for_printer_first_option(opts):
	horizontal, vertical, hscrap, vscrap = get_mounted_pieces_for_printer_first_option(opts)
	return cint(horizontal) * cint(vertical), hscrap, vscrap

def get_total_mounted_pieces_for_printer_second_option(opts):
	horizontal, vertical, hscrap, vscrap = get_mounted_pieces_for_printer_second_option(opts)
	return cint(horizontal) * cint(vertical), hscrap, vscrap

def get_mounted_pieces_for_printer_first_option(opts):
	opts = frappe._dict(opts)

	abstract_horizontal = flt(opts.width) / (flt(opts.final_width) + flt(opts.margin_width))
	abstract_vertical = flt(opts.height) / (flt(opts.final_height) + flt(opts.margin_height))
	
	horizontal_scrap = get_scrap(abstract_horizontal)
	vertical_scrap = get_scrap(abstract_vertical)
	
	percentage_vertical_scrap = get_percentage_scrap(opts.width, horizontal_scrap)
	percentage_horizontal_scrap = get_percentage_scrap(opts.height, vertical_scrap)
...
def get_mounted_pieces_for_printer_second_option(opts):
	...
	abstract_horizontal = flt(opts.width) / (flt(opts.final_height) + flt(opts.margin_height))
	abstract_vertical = flt(opts.height) / (flt(opts.final_width) + flt(opts.margin_width))
...
def get_total_mounted_pieces_for_printer_the_best_option(opts):
	first_option, fhscrap, fvscrap = get_total_mounted_pieces_for_printer_first_option(opts)
	second_option, shscrap, svscrap = get_total_mounted_pieces_for_printer_second_option(opts)
