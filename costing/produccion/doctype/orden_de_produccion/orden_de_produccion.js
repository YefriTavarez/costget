...
frappe.ui.form.on("Orden de Produccion", {
	...
	"setup_and_show_prompt": (frm) => {
		...
		let oncomplete = (form) => {
			frm.set_value("project", form.project);
			frm.set_value("project_name", form.project_name);
		};
	...
	"add_fetchs": (frm) => {
		$.map([
			{"link_field": "customer", "source_field": "customer_name", "target_field": "customer_name"},
		], (value) => frm.add_fetch(value.source_field, value.source_field, value.target_field));
	},
	"project": (frm) => {
		if (frm.doc.project) {
			frm.call("update_project_info").done((response) => {
				frappe.run_serially([
					() => frm.refresh_fields(),
					() => frm.trigger("refresh_section_labels"),
					() => frm.trigger("clear_empty_integer_fields"),
	...
	"printing_dimension": (frm) => {
		if (frm.doc.printing_dimension) {
			frm.call("printing_dimension_change")
				.then((response) => frm.refresh_fields());
		}
	},
	"set_queries": (frm) => {
		...
		$.map([
			{"fieldname": "pre_station", "workstation_type": "Preparacion"},
			{"fieldname": "printer_station", "workstation_type": "Prensa"},
			{"fieldname": "cutter_station", "workstation_type": "Corte"},
			{"fieldname": "options_control_station", "workstation_type": "Control"},
			{"fieldname": "options_protection_station_1", "workstation_type": "Proteccion"},
			{"fieldname": "options_protection_station_2", "workstation_type": "Proteccion"},
			{"fieldname": "options_cutter_station", "workstation_type": "Corte"},
			{"fieldname": "options_union_station", "workstation_type": "Empalme"},
			{"fieldname": "options_packing_station", "workstation_type": "Empaque"},
			{"fieldname": "options_folding_station", "workstation_type": "Plegado"},
			{"fieldname": "options_utility_station_1", "workstation_type": "Utilidad"},
			{"fieldname": "options_utility_station_2", "workstation_type": "Utilidad"},
			{"fieldname": "options_utility_station_3", "workstation_type": "Utilidad"},
			{"fieldname": "options_texture_station_1", "workstation_type": "Textura"},
			{"fieldname": "options_texture_station_2", "workstation_type": "Textura"}
